 ...
 */
$(function() {
	var url = $('.pageUrls').data('contacts');//"{% url 'dashboard:message-contacts' %}";
	var composeUrl = $('.pageUrls').data('compose');// "{% url 'dashboard:compose_message' %}";
	var sendSms = $('#sendSms');
...
function alertUser(msg,status)
{
    $.jGrowl(msg, 
    {header: 'Well done!',theme: status});
}
...
sendSms.on('click',function(){   
    ...
    if(!verb)
    { 
        alertUser('Message Subject required','bg-danger');
        return false;
        subject.focus();
    }
    if(!what){ alertUser('Message Body required','bg-danger'); return false;}
    if(!ccontacts){ ccontacts = false; }
    if(!scontacts){ scontacts = false; } 
    if(!scontacts && !ccontacts && !ucontacts)
    {
        alertUser('Enter at least one contacts!','bg-danger');
        return false;
    }
