	...
	if date:
		try:
			...
			if p2_sz and date:
				...
				return TemplateResponse(request, 'dashboard/reports/product_sales/paginate.html', {'sales': sales, 'gid': date})
		...
		try:
			...
			if p2_sz:
				paginator = Paginator(sales, int(p2_sz))
				sales = paginator.page(page)
				return TemplateResponse(request, 'dashboard/reports/product_sales/paginate.html', {'sales': sales,
...
def sales_search(request):
	if request.is_ajax():
		...
		if request.GET.get('gid'):
			# date = DateFormat(request.GET.get('gid')).format('Y-m-d')
			date = request.GET.get('gid')
		else:
			try:
				last_sale = Sales.objects.latest('id')
		...
		if q is not None:
			...
			if p2_sz:
				...
				return TemplateResponse(request, 'dashboard/reports/product_sales/paginate.html',
										{'sales': sales, 'date': datetime.datetime.strptime(date, '%Y-%m-%d').strftime('%b %d, %Y')})
