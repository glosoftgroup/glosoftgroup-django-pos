...
def list(request):
    try:
        options = Bank.objects.all().order_by('-id')
...
def searchs(request):
    if request.is_ajax():
        ...
        if q is not None:
            ...
            if p2_sz:
                options = paginator.page(page)
                return TemplateResponse(request, 'dashboard/banking/paginate.html', {'options': options,'sz':sz})
            data = {'options': options,
                    'pn': paginator.num_pages,
                    'sz': sz,
                    'q': q}
            return TemplateResponse(request, 'dashboard/banking/search.html', data)
...
def paginate(request):
    ...
    if request.GET.get('gid'):
        options = Bank.objects.filter(name=type.name)
        if p2_sz:
        ...
        if list_sz:
            ...
            return TemplateResponse(request, 'dashboard/banking/p2.html',data)
        ...
        try:
            ...
            if p2_sz:
                ...
                return TemplateResponse(request, 'dashboard/banking/paginate.html', data)
