...
def allocate_list(request):
	try:
		...
		info_logger.info('User: ' + str(request.user.name) + ' accessed the view credit sales report page')
		return TemplateResponse(request, 'dashboard/reports/allocate/sales_list.html',{'pn': paginator.num_pages, 'sales': total_sales, "total_sales_amount":total_sales_amount, "total_tax_amount":total_tax_amount,"date":last_date_of_sales})
	except ObjectDoesNotExist as e:
		error_logger.error(e)
